@extends('layouts.master', ['title' => trans_choice('modules.merchant', 2), 'body' => 'enduser'])

@section('content')

<div class="container" style="padding-top: 7rem; padding-bottom: 5rem;">

    <form action="{{ route('merchant.verifications.store') }}" method="post" role="form" enctype="multipart/form-data">
        @csrf
            ...
            <div class="col-12 ">
                <div class="card card-body shadow">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="form-group">
                                <label for="service" class="col-form-label">{{ __('labels.service') }} <span class="text-danger">*</span></label>
                                <select name="service" id="service" class="form-control select2 @error('service') is-invalid @enderror">
                                    ...
                                    <option value="{{ $service->id }}" {{ collect(old('service', $user->userDetail->service_id ?? null))->contains($service->id) ? 'selected' : null }}>{{ $service->name }}</option>
                                    @empty
                                    @endforelse
                                </select>
                                @error('service')
