@extends('merchant.layouts.master', [ 'title' => trans_choice('modules.product', 2)])
...
<div class="container-fluid">
    ...
    <div class="row my-3">
        <div class="col-12">
            <p class="h5 d-inline">{{ __('labels.available_add_on') }} :</p>
            <button type="submit" class="btn btn-outline-primary btn-rounded-corner float-md-right" form="purchaseForm">{{ __('labels.add_to_cart') }}</button>
        </div>
    </div>

    <form action="{{ route('merchant.carts.store') }}" method="post" enctype="multipart/form-data" role="form" id="purchaseForm">
        @csrf
