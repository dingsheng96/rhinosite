    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="countryStateModal">{{ __('modules.create', ['module' => trans_choice('modules.submodules.country_state', 1)]) }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('settings.countries.country-states.store', ['country' => $country->id]) }}" method="POST" role="form" enctype="multipart/form-data">
                @csrf
