    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="updateCategoryModal">{{ __('modules.edit', ['module' => trans_choice('modules.category', 1)]) }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('categories.update', '__REPLACE__') }}" method="POST" role="form" enctype="multipart/form-data">
                @csrf
                @method('put')
