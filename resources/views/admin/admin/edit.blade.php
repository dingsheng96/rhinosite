    ...
    <div class="row">
        <div class="col-12">
            <div class="card shadow-lg">
                ...
                <form action="{{ route('admin.admins.update', ['admin' => $admin->id]) }}" method="post" role="form" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <select name="role" id="role" class="form-control select2 @error('role') is-invalid @enderror">
                                ...
                                <option value="{{ $role->id }}" {{ old('role', $admin->roles->first()->id) == $role->id ? 'selected' : null }}>{{ $role->name }}</option>
                                @endforeach
                            </select>
                            @error('role')
                            <span class="invalid-feedback" role="alert">
